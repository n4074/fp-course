...
instance Functor MaybeListZipper where
  _ <$> IsNotZ = IsNotZ
  f <$> IsZ (ListZipper l m r) = IsZ $ ListZipper (f <$> l) (f m) (f <$> r)
...
--findLeft p (ListZipper (l :. ls) m r) 
--  | (p l) = IsZ $ leftOne
--  | otherwise = findLeft p leftOne
--  where leftOne = ListZipper ls l (m :. r)

findLeft p (ListZipper l m r) = 
  case break p l of
    (_, Nil) -> IsNotZ
    (x, y :. ys) -> IsZ (ListZipper ys y (reverse x ++ m :. r))
...
--findRight p (ListZipper l m (r :. rs)) 
--  | (p r) = IsZ $ rightOne
--  | otherwise = findRight p rightOne
--  where rightOne = ListZipper (m :. l) r rs

findRight p (ListZipper l m r) = 
  case break p r of
    (_, Nil) -> IsNotZ
    (x, y :. ys) -> IsZ (ListZipper (reverse x ++ m :. l) y ys)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil m r) = let (l :. ls) = (reverse $ m :. r) in ListZipper ls l Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l m Nil) = let (m' :. rs) = (reverse $ m :. l) in ListZipper Nil m' rs
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft z = if hasLeft z then (IsZ $ moveLeftLoop z) else IsNotZ
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight z = if hasRight z then (IsZ $ moveRightLoop z) else IsNotZ
...
swapLeft (ListZipper (l :. ls) m r) = IsZ $ ListZipper (m :. ls) l r
...
swapRight (ListZipper l m (r :. rs)) = IsZ $ ListZipper l r (m :. rs)
...
nth n z = 
  if n < 0 
    then IsNotZ 
      ...
      case moveLeftN' n z of
        Left i -> moveRightN (n - i) z
        Right z'@(ListZipper l _ _) -> 
          let len = length l in 
            if len < n 
              then moveLeftN (len) z 
            else moveRightN (n - len) z'
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l m r) = let m' :. l' = (reverse r ++ (m :. l)) in ListZipper l' m' Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l m r) = let m' :. r' = (reverse l ++ (m :. r)) in ListZipper Nil m' r'
...
-- /Tip:/ Use `zipWith`
  (ListZipper lf mf rf) <*> (ListZipper l m r) = ListZipper (zipWith ($) lf l) (mf m) (zipWith ($) rf r)
...
instance Extend ListZipper where
  (<<=) :: (ListZipper a -> b)
    -> ListZipper a
    -> ListZipper b
  f <<= z = ListZipper (unfoldr (applyAndMove moveLeft) (moveLeft z)) (f z) (unfoldr (applyAndMove moveRight) (moveRight z))
    where applyAndMove move z' = 
            case z' of
              IsNotZ -> Empty
              IsZ z'' -> Full (f z'', move z'') 
...
instance Traversable ListZipper where
  traverse :: Applicative f => 
    ...
    -> ListZipper a
    -> f (ListZipper b)
  traverse f (ListZipper l m r) = lift3 ListZipper (traverse f l) (f m) (traverse f r) 
